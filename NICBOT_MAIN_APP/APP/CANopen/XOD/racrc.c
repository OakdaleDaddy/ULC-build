...
// Initial value for the CRC-16
#define CRC_INIT_VAL  0x0000U

static const UNSIGNED16 mCrc16Tab[256] = {
...
RETURNS: Initial checksum value
**************************************************************************/
UNSIGNED16 CRC_Init (
  void
  )
  ...
  )
{
  ...
  tmp   = *pCRC        ^ ((UNSIGNED16)data & 0x00FFU);
  *pCRC = (*pCRC >> 8) ^ mCrc16Tab[tmp & 0x00FFU];

  return;
}

#endif // (USE_REMOTE_ACCESS == 1)
