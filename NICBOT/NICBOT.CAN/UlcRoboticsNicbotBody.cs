namespace NICBOT.CAN
{
   ...
   public class UlcRoboticsNicbotBody : Device
   {
      ...
      public bool SetSensorAcceleration(UInt32 acceleration)
      {
         bool result = false;

         if (Modes.inspect == this.mode)
         {
            SDODownload download = new SDODownload(0x2350, 0, 4, acceleration);
            result = this.ExchangeCommAction(download);
         }
      ...
      public bool SetSensorHomingVelocity(UInt32 velocity)
      {
         bool result = false;

         if (Modes.inspect == this.mode)
         {
            SDODownload download = new SDODownload(0x2351, 0, 4, velocity);
            result = this.ExchangeCommAction(download);
         }
      ...
      public bool SetSensorHomingBackoff(UInt32 backoff)
      {
         bool result = false;

         if (Modes.inspect == this.mode)
         {
            SDODownload download = new SDODownload(0x2352, 0, 4, backoff);
            result = this.ExchangeCommAction(download);
         }
      ...
      public bool SetSensorTravelVelocity(UInt32 velocity)
      {
         bool result = false;

         if (Modes.inspect == this.mode)
         {
            SDODownload download = new SDODownload(0x2355, 0, 4, velocity);
            result = this.ExchangeCommAction(download);
         }
      ...
      public bool SetSensorServoErrorLimit(UInt16 errorLimit)
      {
         bool result = false;

         if (Modes.inspect == this.mode)
         {
            SDODownload download = new SDODownload(0x2354, 0, 2, errorLimit);
            result = this.ExchangeCommAction(download);
         }
      ...
      public bool SetSensorServoPulsePerDegree(UInt32 pulsePerDegree)
      {
         bool result = false;

         if (Modes.inspect == this.mode)
         {
            SDODownload download = new SDODownload(0x2356, 0, 4, pulsePerDegree);
            result = this.ExchangeCommAction(download);
         }
      ...
      public bool SetSensorIndex(UInt16 sensorIndex)
      {
         bool result = false;

         if (Modes.inspect == this.mode)
         {
            SDODownload download = new SDODownload(0x2315, 2, 2, sensorIndex);
            result = this.ExchangeCommAction(download);
         }
      ...
      public bool SetSensorMotorDisable()
      {
         bool result = false;
         ...
         {
            UInt16 adjustedValue = this.controlWord;
            adjustedValue = (UInt16)(adjustedValue & ~(0x0002));
            result = this.SetControlWord(adjustedValue);
            adjustedValue = (UInt16)(adjustedValue | 0x0002);
            result = this.SetControlWord(adjustedValue);
         }
      ...
      public bool SetSensorHome()
      {
         ...
         {
            UInt16 adjustedValue = this.controlWord;
            adjustedValue = (UInt16)(adjustedValue & ~(0x0001));
            result = this.SetControlWord(adjustedValue);
            adjustedValue = (UInt16)(adjustedValue | 0x0001);
            result = this.SetControlWord(adjustedValue);
         }
      ...
      public bool SetSensorAngle(double angle)
      {
         return (this.SetSensorIndex((UInt16)(angle * 100)));
      }
      ...
      public bool GetSensorAcceleration(ref UInt32 acceleration)
      {
         bool result = false;
         SDOUpload upload = new SDOUpload(0x2350, 0);
         this.pendingAction = upload;
         bool actionResult = this.ExchangeCommAction(this.pendingAction);
      ...
      public bool GetSensorHomingVelocity(ref UInt32 velocity)
      {
         bool result = false;
         SDOUpload upload = new SDOUpload(0x2351, 0);
         this.pendingAction = upload;
         bool actionResult = this.ExchangeCommAction(this.pendingAction);

         if ((false != actionResult) && (null != upload.Data) && (upload.Data.Length >= 4))
         {
            velocity = BitConverter.ToUInt32(upload.Data, 0);
            result = true;
         }
      ...
      public bool GetSensorHomingBackoff(ref UInt32 backoff)
      {
         bool result = false;
         SDOUpload upload = new SDOUpload(0x2352, 0);
         this.pendingAction = upload;
         bool actionResult = this.ExchangeCommAction(this.pendingAction);

         if ((false != actionResult) && (null != upload.Data) && (upload.Data.Length >= 4))
         {
            backoff = BitConverter.ToUInt32(upload.Data, 0);
            result = true;
         }
      ...
      public bool GetSensorTravelVelocity(ref UInt32 velocity)
      {
         bool result = false;
         SDOUpload upload = new SDOUpload(0x2355, 0);
         this.pendingAction = upload;
         bool actionResult = this.ExchangeCommAction(this.pendingAction);
      ...
      public bool GetSensorServoStatus(ref byte status)
      {
         bool result = false;
         SDOUpload upload = new SDOUpload(0x2353, 0);
         this.pendingAction = upload;
         bool actionResult = this.ExchangeCommAction(this.pendingAction);
      ...
      public bool GetSensorServoErrorLimit(ref UInt16 errorLimit)
      {
         bool result = false;
         SDOUpload upload = new SDOUpload(0x2354, 0);
         this.pendingAction = upload;
         bool actionResult = this.ExchangeCommAction(this.pendingAction);
      ...
      public bool GetSensorServoPulsePerDegree(ref UInt32 pulsePerDegree)
      {
         bool result = false;
         SDOUpload upload = new SDOUpload(0x2356, 0);
         this.pendingAction = upload;
         bool actionResult = this.ExchangeCommAction(this.pendingAction);

         if ((false != actionResult) && (null != upload.Data) && (upload.Data.Length >= 4))
         {
            pulsePerDegree = BitConverter.ToUInt32(upload.Data, 0);
            result = true;
         }
