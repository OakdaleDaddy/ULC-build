namespace E4.Ui
{
   ...
   public partial class CANDeviceInformationForm : Form
   {
      ...
      public DeviceClearWarningRequest.ClearWarningHandler OnDeviceClearWarning { set; get; }
      ...
      private void ClearWarningComplete()
      {
         this.ShowDeviceInformation();
         this.ClearWarningButton.Enabled = true;
      }

      private void ProcessDeviceClearWarningComplete(Enum deviceId)
      {
         if (Enum.Equals(deviceId, this.DeviceId) != false)
         {
            this.Invoke((MethodInvoker)(() => { this.ClearWarningComplete(); }));
         }
      }
      ...
      {
         if (null != this.OnDeviceClearWarning)
         {
            this.HbButton.Focus();
            this.ClearWarningButton.Enabled = false;

            DeviceClearWarningRequest.CompleteHandler onComplete = new DeviceClearWarningRequest.CompleteHandler(this.ProcessDeviceClearWarningComplete);
            this.OnDeviceClearWarning(this.DeviceId, onComplete);
         }
         //this.Device.ClearWarning();
      }
