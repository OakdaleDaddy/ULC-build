...
namespace CrossBore.CAN
{
   ...
   public class ElmoWhistleMotor : Device
   {
      ...
      private Int32 requestedVelocity;
      private bool motorStuck;
      private DateTime motorStuckTimeLimit;
      ...
      private void ProcessReadings(Int32 velocityCounts, Int16 torqueCounts)
      {
         ...
         if ((null == this.FaultReason) &&
             (Modes.velocity == this.mode) &&
             (0 != this.requestedVelocity))
         {
            double actualVelocityMagnitude = Math.Abs(this.RPM);
            double requestedVelocityMagnitudeLimit = Math.Abs(this.requestedVelocity) * 0.10;

            if (actualVelocityMagnitude < requestedVelocityMagnitudeLimit)
            {
               if (false == this.motorStuck)
               {
                  this.motorStuck = true;
                  this.motorStuckTimeLimit = DateTime.Now.AddMilliseconds(500);
               }
               else if (DateTime.Now > this.motorStuckTimeLimit)
               {
                  this.SetFault("motor stuck", true);
               }
            }
            else
            {
               this.motorStuck = false;
            }
         }
         else
         {
         ...
         if (nodeId == this.NodeId)
         {
            if (COBTypes.EMGY == frameType)
            {
               ...
               if (0 != errorValue)
               {
                  ...
                  else if (0x3120 == errorCode)
                  {
                     reason = "under-voltage";
                  }
                  else if (0x3310 == errorCode)
                  {
                     reason = "over-voltage";
                  }
                  else if (0x4310 == errorCode)
                  {
                     reason = "temperature stop";
                  }
                  else if (0x5280 == errorCode)
                  {
                     reason = "table problem";
                  }
                  else if (0x5281 == errorCode)
                  {
                     reason = "timing error";
                  }
                  else if (0x5400 == errorCode)
                  {
                     reason = string.Format("cannot start motor ({0:X2})", errorRegister);
                  }
                  else if (0x5441 == errorCode)
                  {
                     reason = "disable by limit switch";
                  }
                  else if (0x6180 == errorCode)
                  {
                     reason = "stack overflow";
                  }
                  else if (0x6181 == errorCode)
                  {
                     reason = "CPU fatal exception";
                  }
                  else if (0x6200 == errorCode)
                  {
                     reason = "user program abort";
                  }
                  else if (0x6300 == errorCode)
                  {
                     reason = "invalid PDO value";
                  }
                  else if (0x6320 == errorCode)
                  {
                     reason = "inconsistent database";
                  }
                  else if (0x7151 == errorCode)
                  {
                     reason = "motor stuck";
                  }
                  else if (0x7300 == errorCode)
                  {
                     reason = "analog encode feedback failure";
                  }
                  else if (0x7380 == errorCode)
                  {
                     reason = "feedback loss";
                  }
                  else if (0x7381 == errorCode)
                  {
                     reason = "two Hall sensors changed";
                  }
                  else if (0x8110 == errorCode)
                  {
                  ...
                  {
                     reason = "peak current exceeded";
                  }
                  else if (0x8380 == errorCode)
                  {
                     reason = "motor electical zero missing";
                  }
                  else if (0x8381 == errorCode)
                  {
                     reason = "cannot tune current offsets";
                  }
                  else if (0x8480 == errorCode)
                  {
                     reason = "speed tracking error";
                  }
                  else if (0x8481 == errorCode)
                  {
                     reason = "speed limit exceeded";
                  }
                  else if (0x8611 == errorCode)
                  {
                     reason = "position tracking error";
                  }
                  else if (0x8680 == errorCode)
                  {
                     reason = "position limit exceeded";
                  }
                  else if (0xFF01 == errorCode)
                  {
                     reason = "request by user program";
                  }
                  else if (0xFF02 == errorCode)
                  {
                     reason = "underflow";
                  }
                  else if (0xFF10 == errorCode)
                  {
                     reason = string.Format("cannot start motor ({0:X2})", errorRegister);
                  }
      ...
      public bool SetVelocity(Int32 velocityRpm)
      {
         ...
         if (false != result)
         {
            this.requestedVelocity = velocityRpm;
         }
